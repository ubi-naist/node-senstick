 ...
 * @param {?string}             error           - エラー内容．エラーが無ければnull．
 * @param {number|undefined}    active_log_num  - 有効ログ数．
 */
 ...
 * @param {?string}             error - エラー内容．エラーが無ければnull．
 * @param {number|undefined}    storage_error_code - 内部ストレージの動作状態．
 ...
 * @param {?string}             error       - エラー内容．エラーが無ければnull．
 * @param {number[]|undefined}  datetime    - SenStick本体の内蔵カレンダーの日付時刻情報．
 * 
 * | Index | Description |
 * | :---- | :---------- |
 * |     0 | 年．0はunknownを示す．|
...
/**
 * SenStickから送られてきたデータを日付時刻情報に変換する．
 * @param {array}                           data        - 変換元のデータ．
 * @param {convertSenstickDatetimeCallback} callback    - コールバック関数． 
 * @private
**/
...
/**
 * SenStickから送られてきたデータを日付時刻情報に変換した際に呼び出されるコールバック関数．
 * @callback convertSenstickDatetimeCallback
 * @param {number}  year        - 年．0はunknownを示す．
 * @param {number}  month       - 月(0から12)．0はunknownを示す．
 * @param {number}  day         - 日(0から<I>month</I>の日数)．0はunknownを示す．
 * @param {number}  hour        - 時(0から23)．
 ...
 * @param {number}  year        - 年(0 <= year)．
 * @param {number}  month       - 月(0 <= month <= 12)．
 * @param {number}  day         - 日(0 <= day <= 31)．
 * @param {number}  hour        - 時(0 <= hour <= 23)．
 * @param {number}  minute      - 分(0 <= minute <= 59)．
 * @param {number}  second      - 秒(0 <= second <= 59)．
 * @returns {?string}           エラー内容．フォーマットに問題が無ければ，null．
 * @private
 */
 ...
 * @param {readLogAbstractTextCallback} callback    - コールバック関数．
 */
...
/**
 * 概要テキストを取得した際に呼び出されるコールバック関数．
 * @callback readLogAbstractTextCallback
 * @param {?string}             error           - エラー内容．エラーが無ければnull．
 ...
 * @param {string}      abstract_text   - 設定する概要テキスト，
 * @param {callback}    callback        - コールバック関数．
 */
...
/**
 * デバイス名を取得する．
 * @param {readSenStickDeviceNameCallback}  callback    - コールバック関数
 */
...
/**
 * デバイス名を取得した際に呼び出されるコールバック関数．
 * @callback readSenStickDeviceNameCallback
 * @param {?string}                 error       - エラー内容．エラーが無ければnull．
 * @param {device_name|undefined}   device_name - デバイス名．
 */

/**
 * デバイス名を設定する
 * @param {string}      device_name - 設定するデバイス名
 * @param {callback}    callback    - コールバック関数
 */
