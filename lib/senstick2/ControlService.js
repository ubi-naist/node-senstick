 ...
 * 内部ストレージのステート情報を取得する
 * @param       callback        コールバック関数
 *              callback関数はfunction(error, storage_error_code)の形を取る．
 */
 ...
 * @param       callback        コールバック関数
 *              callback関数はfunction(error, datetime)の形を取る．
                datetime[0] : year
                datetime[1] : month
                datetime[2] : day
                datetime[3] : hour
 ...
 * @param       year            年
 *              年数をそのまま指定する(ex. 2016)．0はunknownを示す．
 * @param       month           月
 *              1-12月の数値をそのまま指定する(ex. 8)．0はunknownを示す．
 * @param       day             日
 *              1-31日の数値をそのまま指定する(ex. 10)．0はunknownを示す．
 * @param       hour            時
 *              0-23時の数値をそのまま指定する(ex. 13)．
 * @param       minute          分
 *              0-59分の数値をそのまま指定する(ex. 14)．
 * @param       second          秒
 *              0-59秒の数値をそのまま指定する(ex. 01)．
 * @param       callback        コールバック関数
