 ...
 * @param {number[]}                data - 変換元のデータ
 * @param {onAccelerometerChange}   callback - コールバック関数．
 * @private
 */
 ...
 * @param {?string}     error               - エラー内容．エラーが無ければnull．
 * @param {number}      target_log_id       - 読み出し対象のログID．
 * @param {number}      sampling_period     - このログのミリ秒単位のサンプリング周期．
 * @param {number}      measurement_range   - このログの測定レンジ．
 * @param {number}      number_of_samples   - このログに含まれるセンサデータの数．number_of_samplesはサンプル数単位で表される．
 * @param {number}      reading_position    - このログの現在の読み出し位置．reading_positionはサンプル数単位で表される．
 * @param {number}      remaining_storage   - 加速度センササービスのストレージの残量．remaining_storageはサンプル数単位で表される．
 */
 ...
 * notificationを受信した際に自動的に呼び出される．
 * @param {number[]}    data - 受信したデータ
 * @fires SenStick2AccelerometerSensorService#onAccelerometerLogDataChange
 * @private
 */
