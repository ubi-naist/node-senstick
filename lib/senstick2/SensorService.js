...
/**
 * センサのログメタデータを取得する．
 * <b>このメソッドを呼び出す前に，writeSensorLogReadoutTargetIDで対象ログIDをSenStickに送信する必要がある．</b>
...
/**
 * SenStickから送られてきたデータをログメタデータに変換する．
 * @param {Buffer}                              data        - ログメタデータ．
...
/**
 * センサの計測動作を取得した際に呼び出されるコールバック関数．
 * @callback SensorConfigCallback
 * @param {?string}                         error               - エラー内容．エラーが無ければnull．
 * @param {SensorOperationMode|undefined}   operation_mode      - センサー・サービスの動作モード．
 * @param {number|undefined}                sampling_period     - ミリ秒単位のサンプル周期．
 * @param {number|undefined}                measurement_range   - センサの測定レンジ．値の範囲と意味はセンサごとに定義される．
 */
 ...
 * @param {number|undefined}    sampling_period     - ミリ秒単位のサンプリング周期．
 * @param {number|undefined}    measurement_range   - センサの測定レンジ．
 * @param {number|undefined}    number_of_samples   - サンプル数単位のログに含まれるセンサデータ数．
 * @param {number|undefined}    reading_position    - サンプル数単位のログの現在の読み出し位置．
 * @param {number|undefined}    remaining_storage   - サンプル数単位のセンササービスのストレージ残量．
 */
